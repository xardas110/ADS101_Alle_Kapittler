...
template<class T>
void BinaryNode<T>::InOrder(std::list<_In_ BinaryNode*>& container)
{
	if (m_left)
...
template<class T>
void BinaryNode<T>::Slett(BinaryNode* nodePtr)
{
	if (!nodePtr->m_parent)
		return;
	
	BinaryNode** nodeAdresse = nodePtr->m_parent->m_left == nodePtr ? &nodePtr->m_parent->m_left : &nodePtr->m_parent->m_right;
	auto nodePtrLeft = nodePtr->m_left;
	auto nodePtrRight = nodePtr->m_right;

	//case 1: leaf node
	if (!nodePtrLeft && !nodePtrRight)
	{
		*nodeAdresse = nullptr;
		return;
	}
	...
	if (nodePtrLeft && !nodePtrRight)
	{
		*nodeAdresse = nodePtrLeft;
		nodePtrLeft->m_parent = nodePtr->m_parent;
		return;	
	}
	
	if (nodePtrRight && !nodePtrLeft)
	{
		*nodeAdresse = nodePtrRight;
		nodePtrRight->m_parent = nodePtr->m_parent;
		return;
	}
	
	//case 3: multiple child nodes
	std::list<BinaryNode*> inOrderListe;
	nodePtr->InOrder(inOrderListe);
	
	auto erstatter = inOrderListe.back();

	if (erstatter->m_right || erstatter->m_left)
	{
		erstatter->m_left = nullptr;
		inOrderListe.pop_back();
		erstatter = inOrderListe.back();
		erstatter->m_right = nodePtr->m_right;
		erstatter->m_right->m_parent = erstatter;
	}

	erstatter->m_left = nodePtr->m_left;
	erstatter->m_parent = nodePtr->m_parent;
	erstatter->m_left->m_parent = erstatter;
	*nodeAdresse = erstatter;
}
