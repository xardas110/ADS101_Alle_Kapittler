...
int main()
{
	...
	graf.settinn_kant('B', 'D', 6.f);
	
	graf.settinn_kant('C', 'B', 4.f);
	graf.settinn_kant('C', 'D', 1.f);
	
	graf.settinn_kant('A', 'D', 30.f);
...
Vei Graf::Dijkstra(_In_ Node* start, _In_ Node* end)
{
	...
	while (!pq.empty() && !end->m_besøk)
	{
		...
		auto* endNode = vei.kanter.back().m_tilNode;
		
		if (vei.kanter.back().m_tilNode->m_besøk)
			continue;
